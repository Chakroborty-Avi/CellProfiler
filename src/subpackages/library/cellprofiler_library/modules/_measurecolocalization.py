...
def measure_colocalization(args):
    # TODO: Implement 3 options: Image only, Object only, Image and Object
    pass
...
def run_image_pair_images(
    ...
    **kwargs
    ) -> Tuple[List[Tuple[str, str, str, str, str]], Dict[str, float]]:
...
def run_image_pair_objects(
    ...
    labels:                 Annotated[ObjectLabelsDense, Field(description="Labels")],
...
mask:                       Annotated[Optional[ImageGrayscaleMask], Field(description="Mask")] = None, 
    ...
    **kwargs
    ...
    if n_objects == 0:
        ...
        if MeasurementType.CORRELATION in measurement_types:
            res, corr = measure_correlation_and_slope_from_objects(first_pixels, second_pixels, labels, lrange, first_image_name, second_image_name, object_name)
            result += res
        ...
        if set(measurement_types).intersection({MeasurementType.MANDERS, MeasurementType.RWC, MeasurementType.OVERLAP}) != set():
            ...
            (
                ...
                tot_si_thr, 
                combined_thresh,
            ) = get_thresholded_images_and_counts_from_objects(first_pixels, second_pixels, im1_threshold, im2_threshold, labels)

            if MeasurementType.MANDERS in measurement_types:
                res, M1, M2 = measure_manders_coefficient_from_objects(labels, lrange, first_image_name, second_image_name, object_name, fi_thresh, si_thresh, tot_fi_thr, tot_si_thr, combined_thresh)
                result += res


            if MeasurementType.RWC in measurement_types:
                res, RWC1, RWC2 = measure_rwc_coefficient_from_objects(first_pixels, second_pixels, labels, lrange, first_image_name, second_image_name, object_name, fi_thresh, si_thresh, tot_fi_thr, tot_si_thr, combined_thresh)
                result += res

            if MeasurementType.OVERLAP in measurement_types:
                res, overlap, K1, K2 = measure_overlap_coefficient_from_objects(first_pixels, second_pixels, labels, lrange, first_image_name, second_image_name, object_name, combined_thresh)
                result += res
        ...
        if MeasurementType.COSTES in measurement_types:
            ...
            costes_method = kwargs.get("costes_method", CostesMethod.FAST)
            assert costes_method in CostesMethod.__members__.values()
            res, C1, C2 = measure_costes_coefficient_from_objects(first_pixels, second_pixels, labels, lrange, first_image_name, second_image_name, object_name, fi, si, first_image_scale, second_image_scale, costes_method, combined_thresh)
            result += res
